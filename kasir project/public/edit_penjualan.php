...
if (isset($_GET['id'])) {
    ...
    // Fetch the sale record based on id_penjualan
    $stmt = $pdo->prepare("SELECT p.*, b.nama_barang, pl.nama_pelanggan FROM penjualan p
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_sale'])) {
    ...
    $stmt = $pdo->prepare("UPDATE penjualan SET tanggal_penjualan = ?, total_harga = ?, id_pelanggan = ?, id_barang = ?, harga = ?, jumlah = ? WHERE id_penjualan = ?");
    if ($stmt->execute([$tanggal_penjualan, $total_harga, $id_pelanggan, $id_barang, $harga, $jumlah, $id_penjualan])) {
        $success_message = "Sales record updated successfully.";
    } else {
        $error_message = "Error updating sales record.";
    }
}
...
<head>
    ...
    <title>Edit Penjualan - Aplikasi Kasir</title>
    ...
    <div class="flex">
        <div class="w-64 bg-green-800 text-white h-screen">
            ...
            <ul class="mt-6 space-y-4">
                ...
                <li>
                    <a href="penjualan.php" class="block px-4 py-2 text-gray-300 hover:bg-gray-700 rounded">Penjualan</a>
                </li>
                <li>
        ...
        <div class="flex-1 p-6">
            <div class="bg-white shadow-md flex justify-between items-center p-4 rounded-lg mb-6">
                <div class="text-xl font-semibold">Edit Penjualan</div>
            </div>
            ...
            <div class="bg-white p-6 rounded-lg shadow-md">
                ...
                <form method="POST">
                    <div class="mb-4">
                        ...
                        <input type="date" id="tanggal_penjualan" name="tanggal_penjualan" class="mt-1 block w-full p-2 border border-gray-300 rounded-md" value="<?= $sale['tanggal_penjualan'] ?>" required>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select id="id_barang" name="id_barang" class="mt-1 block w-full p-2 border border-gray-300 rounded-md" required>
                            <option value="<?= $sale['id_barang'] ?>"><?= $sale['nama_barang'] ?></option>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" id="harga" name="harga" class="mt-1 block w-full p-2 border border-gray-300 rounded-md" value="<?= $sale['harga'] ?>" step="0.01" required>
                    </div>

                    <div class="mb-4">
                        <label for="jumlah" class="block text-sm font-medium text-gray-700">Jumlah</label>
                        <input type="number" id="jumlah" name="jumlah" class="mt-1 block w-full p-2 border border-gray-300 rounded-md" value="<?= $sale['jumlah'] ?>" required>
                    </div>

                    <div class="mb-4">
                        <label for="total_harga" class="block text-sm font-medium text-gray-700">Total Harga</label>
                        <input type="number" id="total_harga" name="total_harga" class="mt-1 block w-full p-2 border border-gray-300 rounded-md" value="<?= $sale['total_harga'] ?>" step="0.01" required>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select id="id_pelanggan" name="id_pelanggan" class="mt-1 block w-full p-2 border border-gray-300 rounded-md" required>
                            <option value="<?= $sale['id_pelanggan'] ?>"><?= $sale['nama_pelanggan'] ?></option>
                    ...
                    <button type="submit" name="edit_sale" class="w-full bg-blue-500 text-white p-2 rounded-md">Update Sale</button>
                </form>
            </div>
        </div>
    </div>
